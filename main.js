...
function gameLoop() {
  ...
  if (playerPosX < 0) {
    // if player goes beyond left boundary, appear on the right
    playerPosX = tilesCount - 1;
  }
  if (playerPosX > tilesCount - 1) {
    // if player goes beyond right boundary, appear on the left
    playerPosX = 0;
  }
  if (playerPosY < 0) {
    // if player goes beyond top boundary, appear at the bottom
    playerPosY = tilesCount - 1;
  }
  if (playerPosY > tilesCount - 1) {
    // if player goes beyond bottom boundar, appear at the top
    playerPosY = 0;
  }
  ...
  for (let i = 0; i < snakeBoxes.length; i++) {
    screen.fillRect(
      snakeBoxes[i].x * tilesCount,
      snakeBoxes[i].y * tilesCount,
      tilesCount - 2,
      tilesCount - 2
    );
  ...
  screen.fillRect(
    applePosX * tilesCount,
    applePosY * tilesCount,
    tilesCount - 2,
    tilesCount - 2
  );
...
function onKeyUp(event) {
  switch (event.keyCode) {
    case 37: // left
      if (xVelocity != 1) {
        // it handles the opposite movement keypress of snake
        xVelocity = -1; // go left
